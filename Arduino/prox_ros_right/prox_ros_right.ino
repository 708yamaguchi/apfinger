...
ros::Publisher proximity_sensor_pub("proximity_sensors/right/all", &proximity_msg);
...
void loop()
{
  // nh.loginfo("loop start right");
  unsigned long time;
  time = millis();
  // several multiplexer
  measure_proximity(0x70, 4, 0);
  measure_proximity(0x73, 8, 4);
  measure_proximity(0x74, 8, 12);
  measure_proximity(0x71, 8, 20);
  measure_proximity(0x72, 8, 28);
